...
// 自動投稿の基本TTLはAUTO_POST_EXPIRES_AT_MILLISECONDS(24時間)。残り時間が
// この閾値を下回った時だけexpiresAtを書き直すことで、cronが実行される
// たびに無条件でFirestore writeが発生するのを防ぐ。
//
// Ver1.8 Phase2 STEP5-G(2026-09-12実測により変更)｜設計時は
// .github/workflows/main.ymlのcron設定("*/30 * * * *")から「30分毎に
// 確実に実行される」ことを前提に2時間で運用していたが、実際のGitHub
// Actions実行履歴(直近100run、約17日分)を確認した結果、
// 「30分毎」という設定値は実際の実行間隔を保証しないことが判明した
// (最短0.91時間、中央値3.67時間、平均4.01時間、最大12.92時間、
// 24時間超の空白は0件)。そのため「cronは30分毎に確実に動く」ことを
// 鮮度設計の前提にするのをやめ、閾値を12時間に引き上げる。
//
// 12時間は実測された最大観測間隔(12.92時間)を完全にカバーする値では
// なく、絶対にTTL切れを防げる保証値でもない。GitHub Actionsのスケジュール
// 実行自体に確実性の保証が無いため(ドキュメント上も遅延・間引きが
// 明記されている)、この仕組み単体で完全な安全を作ることはできない。
// 12時間は、2時間よりも大幅に安全余裕を増やしつつ、24時間TTLという
// 「短いTTLで鮮度を保つ」という設計思想を保ち、かつ30分毎の無駄な
// writeを避けるための暫定的な運用バランス値である。cronのスケジュール
// 設定("*/30 * * * *")自体は変更していない(GitHub Actions側の実行遅延
// 問題を別の仕組みで解決しようとはしていない)。
